...
export default function ChatCard({ chat, receiver }) {
  ...
  const shouldShowReadMore = !isFileMessage && fullLastMessage.length > messageLimit;
      ...
      <CardContent className="p-3">
        <div className="flex items-center gap-3 w-full min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div
              className={`flex items-center justify-between gap-2 ${chat.unread_count > 0 ? "text-primary font-semibold" : "text-muted-foreground"}`}
            >
              <div className="text-sm flex items-center min-w-0 flex-1">
                {isFileMessage ? (
                  ...
                  <span className="truncate block min-w-0">{previewMessage}</span>
                )}
                <Dot className="shrink-0" />
                <span className="text-xs shrink-0">{chat.time}</span>
              </div>
              {chat.unread_count > 0 && (
                <span className="ml-2 shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-secondary text-xs text-primary-foreground">
                  {chat.unread_count}
                </span>
              )}
            </div>
